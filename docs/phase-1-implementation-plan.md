...
>
> **Deferred:** real `contentHash` dedupe on Document — column exists (`@unique`, nullable), populated by the worker in M3. AnalysisJob → done/failed cleanup paths land in M5. Vector dim assertion (`vector.length === EMBED_DIM`) lands in M3 when real Voyage calls can return a misconfigured dim.
